  ...
  public class UpdateMatriculationDataRequestValidator : AbstractValidator<UpdateMatriculationDataRequest>
  {
    ...
    public UpdateMatriculationDataRequestValidator(ITModeratorDL _moderatorDL)
    {
      RuleFor(x => x.MorningTesters)
          .GreaterThanOrEqualTo(0).WithMessage("MorningTesters cannot be negative.");

      RuleFor(x => x.EveningTesters)
          .GreaterThanOrEqualTo(0).WithMessage("EveningTesters cannot be negative.");

      RuleFor(x => x)
          .Must(x => x.MorningTesters > 0 || x.EveningTesters > 0)
          .WithMessage("At least one of MorningTesters or EveningTesters must be greater than 0.");
